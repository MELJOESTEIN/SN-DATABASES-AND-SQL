...
Reconnaissance : Comprendre la structure de la base de données.
Failed Infiltrations : Analyser les tentatives de connexion échouées.
After Hours Activity : Examiner les connexions hors heures de travail.
Insider Threat : Évaluer les menaces internes dans le département IT.
Cross-Reference Investigation : Croiser les données pour détecter des anomalies.
Comprehensive Security Audit : Fournir un rapport global et des recommandations.
...
Importation du fichier database.sql dans la base organization :mysql -u root -p organization < /home/muhammad/Downloads/SN-DATABASES-AND-SQL/database.sql


Exécution de requêtes SQL pour chaque étape.
Exportation des résultats dans /home/muhammad/SN-DATABASES-AND-SQL/results.txt :mysql organization -e "SELECT ..." >> /home/muhammad/SN-DATABASES-AND-SQL/results.txt
...
Colonnes de log_in_attempts :Field       Type         Null  Key  Default  Extra
----------  -----------  ----  ---  -------  ------------
...
Colonnes de employees :Field         Type         Null  Key  Default  Extra
------------  -----------  ----  ---  -------  ------------
...
Patterns horaires/pays :hour  country  failed_count
----  -------  ------------
19    MEX      1
20    CAN      1
22    UK       1
...
Réussies vs échouées :success  attempt_count
-------  -------------
0        3
1        1
...
Département avec le plus d’échecs :department  failed_count
----------  ------------
Sales       1


Pays non-bureaux :country  failed_count
-------  ------------
MEX      1
CAN      1
UK       1
...
Patterns départements/horaires :department  hour  attempt_count
----------  ----  -------------
Sales       19    1
Finance     20    1
HR          22    1
...
Rapport global :username  department  office      login_date  login_time  success  country  ip_address    device_id
--------  ----------  ----------  ----------  ----------  -------  -------  ------------  ---------
...
Corrélations :department  office      success_count  failed_count  success_rate
----------  ----------  -------------  ------------  ------------
...
Absence de menace interne dans IT :
...
Liste blanche géographique :

Bloquer les connexions depuis des pays non-bureaux (MEX, CAN, UK) via un pare-feu :iptables -A INPUT -s 192.168.1.2 -j DROP  # Exemple pour MEX


Utiliser un WAF (par exemple, ModSecurity) pour une protection avancée.


Surveillance hors heures :

Configurer des alertes dans un SIEM (ELK Stack, Splunk) pour les connexions avant 9h ou après 17h :mysql organization -e "SELECT * FROM log_in_attempts WHERE HOUR(login_time) < 9 OR HOUR(login_time) > 17;" > /home/muhammad/SN-DATABASES-AND-SQL/after_hours.log
